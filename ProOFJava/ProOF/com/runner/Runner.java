...
public class Runner {
    ...
    public Runner(File file, File input_dir, Factory<Run> obj) throws FileNotFoundException, Exception {
        ...
        exec = obj.build_runner(sets[0].class_name, sets[0].name, sets[0].chooses[0]);
        ...
        while(winA.n_servs()>0){
            ...
            while(winA.n_facts()>0){
                ...
                if(factorys==null || factorys.length==0){
                    ...
                    for(Fact f : factorys){
                        for(int c : f.chooses){
                            Approach s = fact.build_runner(f.class_name, f.name, c);
                            if(s==null){
        ...
        WinNeeds winb = new WinNeeds(nodes, ClassIDs);
        for(No no : nodes){
            winb.start(no);
    ...
    private static class WinNeeds implements LinkerApproaches{
        ...
        public WinNeeds(No[] all_nodes, ArrayList<String> ClassIDs) {
        ...
        public <T extends Approach> T need(Class type, T b) throws Exception {
            ...
            if(list.size()==0){
                ...
                LinkedList<Approach> conflict = new LinkedList<Approach>();              
                for(Approach app : list){
                    if(ClassIDs.contains(app.class_id())){
                        conflict.addLast(app);
                    }
                }
                if(conflict.size()==0){
                    throw new Exception("Conflict not resolved: there is no component "+type.getSimpleName()+" selected for this task.\nlist = "+conflict);
                }else if(conflict.size()>1){
                    throw new Exception("Conflict not resolved: there is more than one component "+type.getSimpleName()+" selected for this task.\nlist = "+conflict);
                }
                activate.add(conflict);
                return (T)conflict.getFirst();
            }
            activate.add(list);
    ...
    private static class WinParams extends LinkerParameterAbstract{
        ...
        @Override
        public boolean[] ListBool(String name, String itens[], String description) throws Exception {
            String v[] = params[get++].split(";");
            if(v.length != itens.length){
                throw new Exception("v.length("+v.length+") != itens.length("+itens.length+")"); 
            }
            boolean vals[] = new boolean[v.length];
            for(int i=0; i<v.length; i++){
                vals[i] = Boolean.parseBoolean(v[i]);
            }
            return vals;
        }
        @Override
